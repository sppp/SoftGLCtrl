...
static inline void interpolate(GLVertex *q, GLVertex *p0, GLVertex *p1, double t) {
...
/* Line Clipping algorithm from 'Computer Graphics', Principles and
   Practice */
static inline int ClipLine1(double denom, double num, double *tmin, double *tmax) {
	double t;
...
void gl_draw_line(GLContext *c, GLVertex *p1, GLVertex *p2) {
	double dx, dy, dz, dw, x1, y1, z1, w1;
	double tmin, tmax;
	GLVertex q1, q2;
	int cc1, cc2;
...
static double name(V4 *c,V4 *a,V4 *b) \
{\
  double t,dX,dY,dZ,dW,den;\
  dX = (b->X - a->X);\
  dY = (b->Y - a->Y);\
  dZ = (b->Z - a->Z);\
  dW = (b->W - a->W);\
...
static inline void updateTmp(GLContext *c,
		GLVertex *q, GLVertex *p0, GLVertex *p1, double t) {
...
void gl_draw_triangle(GLContext *c,
	...
	if (co == 0) {
		...
		norm = (double)(p1->zp.x - p0->zp.x) * (double)(p2->zp.y - p0->zp.y) -
			   (double)(p2->zp.x - p0->zp.x) * (double)(p1->zp.y - p0->zp.y);
