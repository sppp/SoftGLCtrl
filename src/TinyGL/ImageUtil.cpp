...
void gl_resizeImage(unsigned char *dest, int xsize_dest, int ysize_dest,
	...
	double x1, y1, x1inc, y1inc;
	int xi, yi, j, xf, yf, x, y;
	
	pix = dest;
	pix_src = src;
	
	x1inc = (double)(xsize_src - 1) / (double)(xsize_dest - 1);
	y1inc = (double)(ysize_src - 1) / (double)(ysize_dest - 1);
...
void gl_resizeImageNoInterpolate(unsigned char *dest, int xsize_dest, int ysize_dest,
	...
	x1inc = (int)((double)((xsize_src) << FRAC_BITS) / (double)(xsize_dest));
	y1inc = (int)((double)((ysize_src) << FRAC_BITS) / (double)(ysize_dest));
