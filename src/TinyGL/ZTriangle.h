...

{
	...
	double fdx1, fdx2, fdy1, fdy2, fz, d1, d2;
	unsigned short *pz1;
	PIXEL *pp1;
...
#endif
#ifdef INTERP_STZ
	double sz1, dszdx, dszdy, dszdl_min, dszdl_max;
	double tz1, dtzdx, dtzdy, dtzdl_min, dtzdl_max;
#endif
...
#ifdef INTERP_STZ
	{
		double zz;
		zz = (double) p0->z;
		p0->sz = (double) p0->s * zz;
		p0->tz = (double) p0->t * zz;
		zz = (double) p1->z;
		p1->sz = (double) p1->s * zz;
		p1->tz = (double) p1->t * zz;
		zz = (double) p2->z;
		p2->sz = (double) p2->s * zz;
		p2->tz = (double) p2->t * zz;
